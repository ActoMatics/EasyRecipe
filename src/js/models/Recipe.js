...
import { key, getRecipeUrl } from '../config';
...
export default class Recipe {
    ...
    async getRecipe() {
        try {
            const res = await axios.get(getRecipeUrl + `?key=${key}&rId=${this.id}`);
    ...
    parseIngredients() {
        ...
        const newIngredients = this.ingredients.map(el => {
            ...
            if (unitIndex > -1) {
                ...
                // example: 4  cups, arrCount is [4] --> "4"
                ...
                // there is no number in first position
                objIng = {
                    count: 1,
                    unit: '',
                    ingredient
